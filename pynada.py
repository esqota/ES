...
GET_EMAIL_URL_TEMPLATE = BASE_API_URL + "/messages/html/{}"
...
class PynadaEmail(object):
    ...
    def get_contents(self):
        if self.contents == None:
            ...
            # Find all <div> elements with the specified styles
            #div_elements = soup.find_all('div', style="color:#D90007; text-decoration: none;")

            # Extract the text content from each <div> element
            #extracted_contents = [div.get_text(strip=True) for div in div_elements]
            
            # Find all <a> elements with href starting with the specified value
            links = soup.select('a[href^="https://mega.nz/#confirm"]')
